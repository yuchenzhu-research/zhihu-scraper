...
- **单链通吃**：支持回答、问题页（自动 Top 抓取）、专栏、创作者主页。
- **本地至上**：直接写出 `Markdown`文件、离线图片目录 (Images) 与 `SQLite`元数据。
- **防反爬机制**：协议优先极速获取，复杂页天然回挂无头浏览器 (Browser Fallback)。
- **实时监控**：不仅能抓一次，还能支持 `monitor` 命令长期增量监控收藏夹更新。
- **现代化 TUI**：忘掉满屏乱飞的日志，使用 `Textual TUI` 管理任务队列、回顾失败记录、一键重试。
