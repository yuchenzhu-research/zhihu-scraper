...
问题和专栏逐项保存，并生成可直接阅读的 `归档进度.md`（同名冲突时自动避让）。终端在每项写入成功后显示进度，来源提供的总数仅作参考。中断后重新运行相同命令即可：程序重新读取列表，复用原目录、稳定文件名及有效媒体，所有项目完成后才发布完整目录或合并问题文档。问题的未合并回答保存在 `回答片段/`；仅输出 PDF 的专栏中间内容保存在 `归档片段/`，成功合并后清理本轮片段。进度文件标注是否完成，旧的完整总目录在失败时保留。

重复归档按知乎内容 ID 复用现有目录和文件路径。标题变化时，正文标题和导航会更新，文件名可能保留旧标题；专栏中已不可访问的文章文件会保留，但本轮目录只列出当前获取的文章。遇到无法确认归属的同名文件或目录时会使用避让名称。

项目刻意不维护数据库、搜索索引或知识图谱。Markdown、媒体和按需生成的 HTML/PDF 就是完整归档，便于用户直接阅读、移动、备份或删除，不产生额外的隐藏状态。
...
批量调用可传入 `progress=callback`，接收 `BatchProgress` 的开始、已保存、完成和中断事件。批量失败抛出 `BatchArchiveInterruptedError`，可读取 `completed` 和 `receipt.progress_path` 定位已保存内容；取消仍抛出 `KeyboardInterrupt`。自定义批量保存器实现 `begin_batch`，返回提供 `write_item`、`finish`、`interrupt` 的会话，详情见 `ArchiveSink` / `BatchArchiveSink`。

## 三平台与开发验证

Windows 会在网络请求前检查输出目录的路径余量；根目录过深时明确报错，请缩短 `archive.output_dir` 后重试。生成新文件名时按 UTF-16 总路径预算处理中文、emoji 和临时文件后缀；已有过长文件保持原样并提示移动到更短目录。
