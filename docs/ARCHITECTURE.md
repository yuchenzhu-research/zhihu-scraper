...
> 状态：2026-09-05 文件优先架构。本文描述已经落地的模块职责和稳定 seam；延期能力见 `FEATURE_TODO.md`。
...
```text
archive_url(URL, settings) → ArchiveReport
```
...
`network.proxy` 贯穿 HTTP/API、项目管理浏览器和媒体下载。外部 CDP 浏览器的代理由该浏览器自己管理。HTTP 重试解析 Retry-After 的秒数和日期，累计等待超过 60 秒或非法等待值会明确停止；连续分页无新增 ID 时停止，即使游标不断变化。认证 HTTP 请求不跨域自动重定向；媒体初始地址、DNS 和每一跳重定向分别校验。HTTP 客户端、浏览器上下文和工作流都具有明确的关闭 seam。
